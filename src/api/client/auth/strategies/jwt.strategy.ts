...
import { AUTH_COOKIE_NAMES, AUTH_JWT_CONFIG } from '../constants/auth.constants';
...
export class JwtStrategy extends PassportStrategy(Strategy, 'jwt') {
  constructor(
    ...
    const secret = configService.get<string>(AUTH_JWT_CONFIG.SECRET_ENV_KEY);
    if (!secret) {
    ...
    super({
      jwtFromRequest: ExtractJwt.fromExtractors([
        (request: Request) => {
          return request?.cookies?.[AUTH_COOKIE_NAMES.ACCESS_TOKEN] ?? null;
        },
      ]),
  ...
  async validate(payload: JwtPayload): Promise<User> {
    ...
    // Check user status - allow ACTIVE only
    const allowedStatuses = [UserStatus.ACTIVE];
